...
public class Purple {
	...
	public static void main(String[] args) {
		PurpleFrame frame = new PurpleFrame(args);
...
class PurpleFrame extends JFrame {
	public PurpleFrame(String[] args) {
		this.setTitle("Draw RedBlue USA");
		this.setSize(1000, 800);

		PurplePanel panel = new PurplePanel();
		panel.setParameter(args);
		this.add(panel);
	}
}

class PurplePanel extends JPanel {
			...
			if (file.isFile() && file.exists()) {
				...
				while ((lineTxt = bufferedReader.readLine()) != null) {
					if (j != 0) {
						...
						float redVote = Integer.parseInt(strs[1]);
						float blueVote = Integer.parseInt(strs[2]);
						float otherVote = Integer.parseInt(strs[3]);
						float sum = redVote + blueVote + otherVote;
						int rValue = (int) Math.floor((redVote / sum) * 255);
						int bValue = (int) Math.floor((blueVote / sum) * 255);
						int gValue = (int) Math.floor((otherVote / sum) * 255);
						data.put(strs[0], new Color(rValue, gValue, bValue));
					}
					j++;
				}
				read.close();
